...
from autods.datasets.domainnet_real import labels_csv

class DomainNetSketch(Dataset):
    ...
    def _make_metadata(self, raw_data_dir: Path):
        ...
        for task_name in self.task_names:
            ...
            for split in ["train", "val"]:
                split_file = list(raw_data_dir.rglob(f"sketch_{split if split == 'train' else 'test'}.txt"))[0]
                image_paths = [path.split(" ")[0] for path in split_file.read_text().split('\n') if len(path) > 0]
                file_tuples = []
