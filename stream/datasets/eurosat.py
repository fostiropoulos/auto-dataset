...
class Eurosat(Dataset):
    metadata_url = "https://github.com/phelber/EuroSAT"
    remote_urls = {
        "EuroSAT.zip": "https://madm.dfki.de/files/sentinel/EuroSAT.zip",
    }
    file_hash_map = {'EuroSAT.zip': 'c8fa014336c82ac7804f0398fcb19387'}
    name = "eurosat"
    dataset_type = "image"
    default_task_name ="none"

    def _process(self, raw_data_dir: Path):
        archive_path = raw_data_dir.joinpath("EuroSAT.zip")
        folder_name = archive_path.stem.lower()
        save_path = raw_data_dir.joinpath(folder_name)
        extract(archive_path, save_path)

    def _make_metadata(self, raw_data_dir: Path):
        images_path = list(raw_data_dir.rglob("*.jpg"))
        # train test split
