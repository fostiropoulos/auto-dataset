...
class Cifar10(Dataset):
    ...
    def _process(self, raw_data_dir: Path):
        ...
        for p in raw_data_dir.rglob("data_batch_*"):
            train_set = self.unpickle(p)
            labels = train_set[b"labels"]
            train_labels = np.array(labels).reshape(
                len(labels), 1
            )
            self._nparray_to_image(
                train_set[b"data"].reshape(-1, 3, 32, 32).transpose(0, 2, 3, 1),
                train_labels,
                "train",
            )

        test_set = self.unpickle(list(raw_data_dir.rglob("test_batch"))[0])
        test_labels = np.array(test_set[b"labels"]).reshape(
            len(test_set[b"labels"]), 1
        )
        self._nparray_to_image(
            test_set[b"data"].reshape(-1, 3, 32, 32).transpose(0, 2, 3, 1),
            test_labels,
            "val",
        )
    ...
    def _make_metadata(self, raw_data_dir: Path):
        ...
        # read class name
        label_names = self.unpickle(list(raw_data_dir.rglob("*.meta"))[0])[
            b"label_names"
        ]
